...
(function ($, _, Backbone, Drupal, drupalSettings) {
  ...
  Drupal.panels_ipe.BlockPicker = Drupal.panels_ipe.CategoryView.extend(/** @lends Drupal.panels_ipe.BlockPicker# */{
    ...
    template_plugin: _.template(
      '<div class="ipe-block-plugin ipe-blockpicker-item <%- dynamic_class %>">' +
     ...
     * @type {function}
     */
    // @@@ Note: AHD-3922 adding alias to change display label in IPE Tray.
    template_create_button: _.template(
      ...
      '  <%- alias %>' +
      '  <% if (count) { %><div class="ipe-category-count"><%- count %></div><% } %>' + '</a>'
    ),
    ...
    render: function () {
      ...
      if (drupalSettings.panels_ipe.user_permission.create_content) {
        var block_types_count = 0;
        if (drupalSettings.panels_ipe.block_types_count) {
          block_types_count = drupalSettings.panels_ipe.block_types_count;
        }
        this.$('.ipe-categories').prepend(this.template_create_button({
          name: 'Create Content',
          alias: 'Templated Component Library',
          count: block_types_count,
    ...
    template_item: function (block_plugin) {
      ...
      template_vars.dynamic_class = '';
      if (template_vars.category == 'Place New Component') {
        template_vars.dynamic_class = 'ipe-hide-block'
      }

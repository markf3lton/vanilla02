...
class BrightcoveTextTrack extends ContentEntityBase implements BrightcoveTextTrackInterface {
  ...
  public static function createOrUpdate(TextTrack $text_track, EntityStorageInterface $storage, $video_entity_id) {
    ...
    if (!empty($video_entity_id)) {
      ...
      if (empty($video)) {
          \Drupal::logger('Brightcove')->error('Brightcove Video with video entity id ' . $video_entity_id . ' is not found.');
          return;
        }
    }
    else {
    ...
    $field_sources = [];
      if (!empty($sources)) {
        foreach ($sources as $source) {
          ...
          \Drupal::logger('Brightcove')->notice('Brightcove text track source not found for text id ' . $text_track->getId());
      }
